...
class ETLProcessor:
    ...
    def __init__(self, origen_config: Dict, destino_config: Dict, paths: Dict, performance_config: Dict):
        """
    ...
    def create_temp_tables(self, query_results: Dict[str, List[Dict]], schema: str) -> List[str]:
        ...
        for filename, data in query_results.items():
            if not data:
                self.logger.logger.warning(f"⚠️ No hay datos para {filename} en esquema {schema}")
                continue
    ...
    def execute_inserts(self, schema: str) -> int:
        """
        ...
        Returns:
            int: Número total de registros insertados
        """
        ...
        self.logger.logger.info(f"📥 Total inserts ejecutados para {schema}: {inserts_executed}")
    ...
    def process_schema(self, schema: str) -> bool:
        """
        Procesa un esquema completo: queries -> temp tables -> inserts -> cleanup
        ...
        try:
            ...
            # 2. Ejecutar queries y obtener datos
            ...
            total_records = self.execute_inserts(schema)
            inserts_count = len(self.read_insert_order())
            ...
            if not temp_tables:
                step = "Ejecución de queries"
            elif temp_tables and total_records == 0:
                step = "Creación de tablas temporales"
            else:
